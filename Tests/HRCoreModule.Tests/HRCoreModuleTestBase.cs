...
namespace HRCoreModule.Tests
{
    public abstract class HRCoreModuleTestBase : AbpIntegratedTestBase<HRCoreModuleTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected HRCoreModuleTestBase()
        {
        ...
        protected void UsingDbContext(Action<HRCoreModuleDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<HRCoreModuleDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<HRCoreModuleDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<HRCoreModuleDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<HRCoreModuleDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HRCoreModuleDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<HRCoreModuleDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HRCoreModuleDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<HRCoreModuleDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<HRCoreModuleDbContext, Task<T>> func)
        {
            T result;
